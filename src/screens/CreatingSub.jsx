import { Image, View, Text, Button, FlatList, ScrollView, Alert } from "react-native";
...
export default function CreatingSub({ navigation }) {
  ...
  return (
    <View className="flex flex-col justify-between items-center mx-4 my-4 h-[88%]">
      <SearchBar />
      <FlatList className="w-full" data={Data} renderItem={({ item }) =>
        <TouchableOpacity onPress={() => {
          navigation.navigate("NewSubscription", { name: item.name, price: item.price, color: item.color })
        }}>
          <Boxsubscription image={item.img} name={item.name} price={item.price} color={item.color} />
        </TouchableOpacity>
      } />
